//VARIÁVEIS
var palavras = ["ALICATE", "MARTELO", "TRENA", "NIVEL", "MARRETA", "CHAVES", "REGUA", "TALHADEIRA", "PONTEIRO", "FURADEIRA", "PRUMO", "SERRA", "BOMBA", "CARRINHO", "ESQUADRO", "SERROTE", "MARTELETE", "CUNHA", "LIMA", "LANTERNA"];
var letras = [];
...
function verificaPalavraDoInput() {
    valorPalavra = (inputAddPalavra.value).toUpperCase();
        if ((valorPalavra.length < 4) || (valorPalavra.length > 10)){
            alert("Digite o nome de uma ferramenta válida e com até 10 letras!");
            inputAddPalavra.value = "";
            inputAddPalavra.focus();

        }
        else if (palavras.includes(valorPalavra)) {
            alert("Essa ferramenta já está na lista, tente outro!")
            inputAddPalavra.value = "";
            inputAddPalavra.focus();
        }
        else{
